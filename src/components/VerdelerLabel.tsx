...
const VerdelerLabel: React.FC<VerdelerLabelProps> = ({ verdeler, projectNumber }) => {
  ...
  return (
    <div className="w-[152.4mm] h-[101.6mm] bg-white p-3 relative flex flex-row" style={{ color: '#000000' }}>
      {/* Left section - Logo and company info */}
      <div className="flex flex-col justify-between pr-3 border-r-2 border-gray-300" style={{ width: '35%' }}>
        <div>
          <img src="/EWP-logo-zwart.png" alt="EWP Logo" className="h-14 object-contain mb-2" style={{ width: 'auto' }} />
          <div className="text-[10pt] leading-tight text-black">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-[9pt] mt-1 text-center text-black font-bold">Scan voor onderhoud</p>
        </div>
      </div>

      {/* Right section - Details */}
      <div className="flex-1 pl-3 flex flex-col justify-center">
        <div className="text-[11pt] grid grid-cols-2 gap-x-4 gap-y-1.5">
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Verdeler ID:</span>
            <span className="text-black font-semibold ml-2">{verdeler.distributor_id || verdeler.distributorId}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Project:</span>
            <span className="text-black font-semibold ml-2">{displayProjectNumber}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Kastnaam:</span>
            <span className="text-black font-semibold ml-2 truncate">{verdeler.kast_naam || verdeler.kastNaam || '-'}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Systeem:</span>
            <span className="text-black font-semibold ml-2 truncate">{verdeler.systeem || '-'}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Voeding:</span>
            <span className="text-black font-semibold ml-2 truncate">{verdeler.voeding || '-'}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Un in V:</span>
            <span className="text-black font-semibold ml-2">{verdeler.un_in_v || verdeler.unInV || '-'}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">In in A:</span>
            <span className="text-black font-semibold ml-2">{verdeler.in_in_a || verdeler.inInA || '-'}</span>
          </div>
          <div className="flex justify-between pr-2">
            <span className="font-bold text-black whitespace-nowrap">Freq. in Hz:</span>
            <span className="text-black font-semibold ml-2">{verdeler.freq_in_hz || verdeler.freqInHz || '-'}</span>
          </div>
        </div>
      </div>
