...
import { Plus, Trash2, Upload, Eye, CheckSquare, Printer, Key, Copy, Clock, Users, CheckCircle, XCircle, AlertTriangle, X, Edit, Save } from 'lucide-react';
...
const VerdelersStep: React.FC<VerdelersStepProps> = ({ 
  ...
  const handleGenerateAccessCode = (verdeler: any) => {
    setShowVerdelerDetails(null); // Close details modal
    setNewAccessCode({
    ...
    setShowAccessCodeForm(verdeler);
  };
  ...
  const handleCreateAccessCode = async () => {
    ...
    try {
      ...
      const accessCodeData = {
        ...
        verdeler_id: showAccessCodeForm.distributorId,
  ...
  const handleShowVerdelerDetails = (verdeler: any) => {
  ...
  const isProjectInOfferte = () => {
    return projectData.status?.toLowerCase() === 'offerte';
  };

  const isProjectInProductie = () => {
    return projectData.status?.toLowerCase() === 'productie' || 
           projectData.status?.toLowerCase() === 'testen' ||
           projectData.status?.toLowerCase() === 'levering' ||
           projectData.status?.toLowerCase() === 'opgeleverd';
  };
                ...
                {verdelers.map((verdeler) => {
                  ...
                  return (
                    <tr 
                      ...
                      onClick={() => handleShowVerdelerDetails(verdeler)}
                    >
                      <td className="py-4">
                      ...
                      <td className="py-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleShowVerdelerDetails(verdeler);
                            }}
          ...
          <div className="bg-[#1E2530] rounded-2xl p-6 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center space-x-4">
                <div>
                  <h2 className="text-xl font-semibold text-green-400">
                    {showVerdelerDetails.distributorId}
                  </h2>
                  <p className="text-gray-400">{showVerdelerDetails.kastNaam || 'Naamloos'}</p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-[#2A303C] p-6 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-white font-medium">{showVerdelerDetails.distributorId}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Kastnaam:</span>
                      <p className="text-white font-medium">{showVerdelerDetails.kastNaam || 'Naamloos'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Toegewezen monteur:</span>
                      <p className="text-white">{showVerdelerDetails.toegewezenMonteur || 'Vrij'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Status:</span>
                      <p className="text-white font-medium">{showVerdelerDetails.status || 'In productie'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Systeem:</span>
                      <p className="text-white">{showVerdelerDetails.systeem || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Voeding:</span>
                      <p className="text-white">{showVerdelerDetails.voeding || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Bouwjaar:</span>
                      <p className="text-white">{showVerdelerDetails.bouwjaar || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Fabrikant:</span>
                      <p className="text-white">{showVerdelerDetails.fabrikant || '-'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#2A303C] p-6 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-white">{showVerdelerDetails.unInV || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">In in A:</span>
                      <p className="text-white">{showVerdelerDetails.inInA || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Ik Th in KA 1s:</span>
                      <p className="text-white">{showVerdelerDetails.ikThInKA1s || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Ik Dyn in KA:</span>
                      <p className="text-white">{showVerdelerDetails.ikDynInKA || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Freq. in Hz:</span>
                      <p className="text-white">{showVerdelerDetails.freqInHz || '-'}</p>
                    </div>
                    <div>
                      <span className="text-gray-400">Type nr. HS:</span>
                      <p className="text-white">{showVerdelerDetails.typeNrHs || '-'}</p>
                    </div>
                  </div>
                </div>
                ...
                {(() => {
                  const verdelerCodes = getVerdelerAccessCodes(showVerdelerDetails.distributorId);
              ...
              <div className="space-y-4">
                <div className="bg-[#2A303C] p-6 rounded-lg">
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={() => handleEditVerdeler(showVerdelerDetails)}
                    ...
                    <button
                      onClick={() => {
                        ...
                        handleDeleteVerdeler(showVerdelerDetails.id);
                      }}
                ...
                <div className="bg-[#2A303C] p-6 rounded-lg">
                  ...
                  <div className="space-y-3">
                    <VerdelerTesting
                      ...
                      onComplete={(testData) => handleTestComplete(showVerdelerDetails, testData)}
                ...
                <div className="bg-[#2A303C] p-6 rounded-lg">
                  ...
                  <div className="space-y-3">
                    ...
                    <button
                      onClick={() => handleGenerateAccessCode(showVerdelerDetails)}
                      className="btn-secondary w-full flex items-center space-x-2"
                    >
                      <Key size={16} />
                  ...
                  {isProjectInOfferte() ? (
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className="text-yellow-400 font-medium">Project in Offerte Fase</span>
                      </div>
                      <p className="text-yellow-300 text-sm">
                        Technische specificaties kunnen alleen worden ingevuld wanneer het project status "Productie" of hoger heeft.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
          ...
          <div className="card p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div className="bg-[#2A303C] p-4 rounded-lg">
                ...
                <p className="text-white">{showAccessCodeForm.distributorId} - {showAccessCodeForm.kastNaam || 'Naamloos'}</p>
