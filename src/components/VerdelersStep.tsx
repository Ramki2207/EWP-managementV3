...
const VerdelersStep: React.FC<VerdelersStepProps> = ({
  ...
  const openChecklistInNewWindow = async (verdeler: any) => {
    ...
    try {
      const dbTestData = await dataService.getTestData(verdeler.id);
    ...
    newWindow.document.write(`
      ...
      <head>
        ...
        <title>Pre-Testing Checklist - ${verdeler.distributor_id}</title>
  ...
  const handleSaveVerdeler = async () => {
    ...
    try {
      ...
      if (verdelerData.status === 'Testen' && editingVerdeler && verdelers.length > 1) {
        ...
        if (updatedVerdeler) {
          // Open checklist directly in new window
          setTimeout(() => {
            openChecklistInNewWindow(updatedVerdeler);
          }, 100);
        }
